...
Mobify.UI.Carousel = (function($, Utils) {
    ...
    Carousel.prototype.bind = function() {
        ...
        function npmType(newId) {
            //#m-carousel_news1
            switch(newId) {
                case "m-carousel_news1":
                    return ".eduActive";
                    break;
                case "m-carousel_news2":
                    return ".latest";
                    break;
                case "m-carousel_news3":   //桌機其他資訊
                    return ".friendLink";
                    break;
                case "m-carousel_news": //行動版最新消息
                    return ".runNews";
                    break;
                case "m-carousel_news4":  //行動版其他資訊
                    return ".friendLink.slideNews.mobileAdd";
                    break;
                //case "m-carousel_news4":
                //    return ".friendLink";
                //    break;
                default:
                    return "";
        ...
        $element.on('afterSlide', function (e, previousSlide, nextSlide) {
            var ctlId = self.element.id;
            var ctlClassName = npmType(ctlId);
          ...
          //  self.$element.find('[data-slide=\'' + nextSlide + '\']').addClass(self._getClass('active'));
            //--20130408阿成:對應上下則按鈕顯示
            //alert(previousSlide + ";" + nextSlide);
            $('.m-carousel-prev').show();
            $('.m-carousel-next').show();
            //$(ctlClassName + '.leftBtn').show();
            //$(ctlClassName + '.rightBtn').show();
            $(ctlClassName).find('.leftBtn').show();
            $(ctlClassName).find('.rightBtn').show();
           // alert(ctlClassName);
            if (self.$items.size() <= 1) {
                $('.m-carousel-prev').hide();
                $('.m-carousel-next').hide();
                $(ctlClassName).find('.leftBtn').hide();
                $(ctlClassName).find('.rightBtn').hide();
                //alert(ctlClassName);
            }
            else {
                if (nextSlide == 1) {
                    $('.m-carousel-prev').hide();
                    $(ctlClassName).find('.leftBtn').hide();
                   // alert('A:'+ ctlClassName);
                }
                else {
                    if (nextSlide+1 == self.$items.size()) {
                        $('.m-carousel-next').hide();
                        $(ctlClassName).find('.rightBtn').hide();
                      //  alert('B:' + ctlClassName);
                    }
                }
            }
            //++
